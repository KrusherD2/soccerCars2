...
fn.worldSky = function() {
	...
	//this.light.angle = 0.75;
	//this.light.exponent = 2.0;
...
fn.physicsAndTextureFromSrc = function(heightmapSrc, textureSrc, callback) {
	var options = {};
	options.xSegments = 128;
	options.ySegments = 128;
	options.xSize = 1024;
	...
	this.loadFile = function(url) {
		...
		localforage.getItem('files.' + url).then(function(value) {
			if (value !== null) {
				...
				console.log("automatically loaded: " + url);
				...
				loader.load(url, function(value) {
					...
					scope.assets.files[url].value = value;
					...
					if(!fn.isMobile()) {
						...
						localforage.setItem('files.'+url, storable).then(function(value) {
							console.log("manually loaded and stored: " + url);
						});
					}
...
fn.zone = function(worldMap, column, row) {//fill in paramater for heightmap url or define url scheme, also paramater for position
	var scope = this;
	...
	this.setMeshPhysTex = function(heightmapURL, textureURL) {
		//scope.heightmap = heightmapURL;
		/*fn.physicsFromHeightmap(heightmapURL, function(mesh, phys) {
			
			phys.position.set(scope.mapPosition.x+(scope.width/2), scope.mapPosition.y+(scope.height/2), 0);
			mesh.position.set(scope.mapPosition.x+(scope.width/2), scope.mapPosition.y+(scope.height/2), 0);
			scope.mesh = mesh;
			scope.phys = phys;
			
			scope.worldMap.world.c.pw.addBody(phys);
			//scope.worldMap.world.t.scene.add(mesh);
		});
		
		fn.planeFromHeightmapSrc(heightmapURL, textureURL, function(mesh) {
			mesh.position.set(scope.mapPosition.x+(scope.width/2), scope.mapPosition.y+(scope.height/2), 0);
			scope.worldMap.world.t.scene.add(mesh);
		});*/
		
		fn.physicsAndTextureFromSrc(heightmapURL, textureURL, function(mesh, phys) {
			scope.mesh = mesh;
			scope.phys = phys;
...
fn.worldMap = function(world) {
	...
	for(i = 0; i < this.columns; i++) {
		for(var j = 0; j < this.rows; j++) {
			this.zones[i][j] = new fn.zone(this, i, j);
...
//world1.game.worldMap.setZoneByCoord(new THREE.Vector2(1, 1));
...
function character() {
	...
	this.update = function() {
		...
		if (this.mesh.animTo !== this.mesh.animPlaying) {
...
function spellSlot(spellBar, width, height, pos, row, column) {
	...
	/*this.changeSpell = function(spellName) {a
